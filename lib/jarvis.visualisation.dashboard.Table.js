...
jarvis.visualisation.dashboard.Table = function (options) {
    ...
    //jarvis.debug.log('INFO', 'jarvis.visualisation.dashboard.Table', 5, '...Constructor (' + executionTime + 'ms)');
};
...
jarvis.visualisation.dashboard.Table.prototype.fetch = function (sender, container) {
    //if (!sender)
    //    sender = jarvis.visualisation.dashboard.Table;
        ...
        $(data).each(function (index, item) {
            try {
                if (item.data.Result.Rows.length > _itemCount - 1) {
                    ...
                    lastRow.FormattedValues[1] = sum1; //jarvis.string.formatNumber(sum1, 0, true);
                    ...
                    if (lastRow.Values.length > 2) {
                        ...
                        lastRow.FormattedValues[2] = sum2;  //jarvis.string.formatNumber(sum2, 0, true);
                    }
