...
jarvis.visualisation.realtime.Timeline = function (options) {
    ...
    //jarvis.debug.log('INFO', 'jarvis.visualisation.realtime.Timeline', 5, '...Constructor (' + executionTime + 'ms)');
};
...
jarvis.visualisation.realtime.Timeline.prototype.init = function (options, container) {
        ...
        if (!$(this).parent().hasClass('prettyprint')) {
            ...
            if (jarvis.visualisation.realtime.connected) {
                ...
                var queryOptions = {
                    ...
                    Period:1,
                    omitDate:false,
                    callback:'jarvisResponse_Realtime_Response'
                };
    ...
    if (jarvis.visualisation.realtime.paused) {
        ...
        //_this.processFetch (_this, {timestamp:jarvis.realtime.pausedTime});
...
jarvis.visualisation.realtime.Timeline.prototype.jarvisResponse_Realtime_Timeline = function (message, sender, timestamp) {
    ...
    if (!_this.initialized || jarvis.visualisation.realtime.paused) {
        ...
        if (!_this.gettingBackData) { //!jarvis.realtime.paused
            ...
            var queryOptions = {
                FromDate:jarvis.date.formatDate(fromDate, 'yyyy-mm-dd hh:nn:00.000'),
                ToDate:jarvis.date.formatDate(toDate, 'yyyy-mm-dd hh:nn:ss.999'),
                Dimensions:'',
                Metrics:'Bet Count',
                Resolution:'Minute',
