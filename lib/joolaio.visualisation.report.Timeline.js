...
joolaio.visualisation.report.Timeline.prototype.init = function (options, container) {
    ...
    _this.disableMetrics = this.options.disableMetrics;
    ...
    $(matchedContainers).each(function (index, item) {
      ...
      var _hidexaxis =  $(item).attr('data-hidexaxis');
      if (_hidexaxis)
        _this.options.hidexaxis = _hidexaxis;
...
joolaio.visualisation.report.Timeline.prototype.fetch = function (sender) {
    ...
    if (_this.disableMetrics.indexOf(_this.primaryMetric.name) != -1 && joolaio.options.timeline.plotDisabledWhenFiltered && _this.Filters.length == 0) {
      var metricName = joolaio.visualisation.reportWrapper.panel.tabs[0].metricgroups[0].metrics[0];
      _this.primaryMetric = _.filter(joolaio.objects.Metrics, function(metric){
        return metric.id== metricName;
      });

      if (Array.isArray(_this.primaryMetric ))
        _this.primaryMetric=_this.primaryMetric[0];

      _this.pickers.primarymetric.setSelected(_this.pickers.primarymetric,_this.primaryMetric.name);
      
      return;
    }  
  
    if (_this.secondaryMetric && _this.disableMetrics.indexOf(_this.secondaryMetric.name) != -1 && joolaio.options.timeline.plotDisabledWhenFiltered && _this.Filters.length == 0) { 
      _this.pickers.secondarymetric.setSelected(_this.pickers.secondarymetric,'');
...
//        series.splice(0, 1);
    $(series).each(function (index, item) {
        ...
        var visible = true;
        if ((item.ordinal == 0 || item.ordinal == 4 || item.ordinal == 1 || item.ordinal == 5) && (!_this.options.showPrimary && _this.Filters.length > 0)){
            visible = false; //
        }
      
        sender.Chart.addSeries({visible: visible, turboThreshold: item.data.length, name: item.name, color: item.color, lineWidth: item.lineWidth, type: item.type, shadow: item.shadow, yAxis: item.yaxis, data: item.data, ordinal: item.ordinal, ytype: item.ytype, pointInterval: 24 * 3600 * 1000}, false);
    });
...
joolaio.visualisation.report.Timeline.prototype.drawChart = function (Container) {
    ...
    var picker_metrics = new joolaio.visualisation.picker.Metrics({
        ...
        disable: _this.options.disableMetrics,
    ...
    _this.pickers.primarymetric = picker_metrics;
    ...
    _this.pickers.secondarymetric = picker_secondary;
    ...
    $(joolaio.visualisation).bind('secondarymetric', function (e, sender, metric) {
        if (sender.type != _this.type) {
          ...
          //picker_metrics.disableMetric(picker_secondary, _this.secondaryMetric);
        }
    });
    ...
    _this.Chart = new Highcharts.Chart({
        ...
        xAxis: {
            ...
            labels: {
                ...
                fontFamily: 'Signika',
                enabled: true, //_this.options.hide_xaxis,
