...
joolaio.visualisation.report.Timeline.prototype.fetch = function (sender) {
    ...
    if (_this.disableMetrics.indexOf(_this.primaryMetric.name) != -1 && joolaio.options.timeline.plotDisabledWhenFiltered && _this.Filters.length == 0) {
      _this.pickers.primarymetric.setSelected(_this.pickers.primarymetric,joolaio.objects.Metrics[0].name);
      _this.primaryMetric = joolaio.objects.Metrics[0];
      console.log('not fetching primaryMetric');
      //console.log(_this);
    ...
    if (_this.secondaryMetric && _this.disableMetrics.indexOf(_this.secondaryMetric.name) != -1 && joolaio.options.timeline.plotDisabledWhenFiltered && _this.Filters.length == 0) { 
      console.log('not fetching secondaryMetric');
      _this.pickers.secondarymetric.setSelected(_this.pickers.secondarymetric,'');
      _this.secondaryMetric = null;
    }
    /*
    if (_this.disableMetrics.indexOf(_this.primaryMetric.name) != -1 && !joolaio.options.timeline.plotDisabledWhenFiltered) {
      console.log('not fetching primaryMetric');
      return;
    }

    if (_this.secondaryMetric && _this.disableMetrics.indexOf(_this.secondaryMetric.name) != -1 && !joolaio.options.timeline.plotDisabledWhenFiltered) {
      console.log('not fetching secondaryMetric');
      return;
    }
    */
