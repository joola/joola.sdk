...
joolaio.visualisation.picker.Dimensions.prototype.disableDimension = function (sender, dimension) {
  ...
  $(container).find('.key').each(function (i, m) {
    ...
    nodeParent.removeClass('disabled');
    var nodeCategory = $(nodeParent).parent().parent();
    var nodeCount = $(nodeCategory).find('.node:not(.disabled)').length;
  ...
  if (dimension && dimension != '') {
    $(container).find('.key').each(function (i, m) {
      ...
      if ($(m).text() == dimension.name) {
        ...
        //check if we need to hide the category
        var nodeCategory = $(nodeParent).parent().parent();
        var nodeCount = $(nodeCategory).find('.node:not(.disabled)').length;
...
joolaio.visualisation.picker.Dimensions.prototype.baseHTML = function (sender) {
  ...
  _dimensions = _.sortBy(_dimensions, function (item) {
    ...
    return item[0].category + item[0].ordinal;
  });
    ...
    if (item[0].category && item[0].category != '(not set)') {
      ...
      $.each(item, function (index, dimension) {
        ...
        if (_this.options.exclude.indexOf(dimension.name) == -1) {
          ...
          list += '<div class="help"> <i class="tipsy icon-question-sign icon-white" data-caption="' + dimension.name + '" title="' + dimension.description + '"></i> </div>';
  ...
  $($container).find('.tipsy').each(function (index, item) {
    try {
      $(item).tooltipsy({
        offset: [10, 0],
        show: function (e, $el) {
      ...
      console.log('failed to assign tooltips: ' + e);
    }
  });
