...
jarvis.visualisation.realtime.Geo = function (options) {
    ...
    //jarvis.debug.log('INFO', 'jarvis.visualisation.realtime.Geo', 5, '...Constructor (' + executionTime + 'ms)');
};
        ...
        if (!$(this).parent().hasClass('prettyprint')) {
            ...
            if (jarvis.visualisation.realtime.connected) {
                ...
                var queryOptions = {
                    ...
                    Period:period,
                    omitDate:true,
                    callback:'jarvisResponse_Realtime_Response_Geo',
                    Limit:limit,
                    SortDir:'DESC',
                    SortKey:metric
                };
...
/*
 jarvis.realtime.visualisation.Table.prototype.processFetch=function (sender, data, error)
 {
 data = $.parseJSON(data.data).Result;
 console.log(data);
 var _this = sender;
