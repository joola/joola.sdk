...
jarvis.objects.Auth.prototype.checkToken = function (sender, options, callback) {
    var result;

    if (typeof callback == 'function')
        jarvis.dataaccess.fetch(this, '/auth.checkToken', options, function (sender, data, error) {
            result = data.authenticated;
            callback(sender, result);
        });
    else {
        result = jarvis.dataaccess.fetch(this, '/auth.checkToken', options, null);
