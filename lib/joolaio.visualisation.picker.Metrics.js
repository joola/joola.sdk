  ...
  _.each(_metrics, function (item, index) {
    ...
    $.each(item, function (index, metric) {
      if (_this.options.exclude.indexOf(metric.name) == -1 && _this.options.disable.indexOf(metric.name) == -1) {
        ...
        list += '<div class="help"> <i class="tipsy icon-question-sign icon-white" data-toggle="tooltip" data-caption="' + metric.name + '" data-text="' + metric.description + '" title="' + (metric.description == null ? 'Information not available.' : metric.description) + '"></i> </div>';
