...
jarvis.visualisation.report.OverviewMetricBox = function (options) {
    ...
    //jarvis.debug.log('INFO', 'jarvis.visualisation.report.OverviewMetricBox', 5, '...Constructor (' + executionTime + 'ms)');
};
...
jarvis.visualisation.report.OverviewMetricBox.prototype.update = function (sender, metric, series, container) {
    ...
    $(series[0]).each(function (si, so) {
        ...
        if (metric.AggregationType == "AVG") {
            ...
            if (series.length == 1)
                $metric.find('.site').html('Overall Avg: <span class="summaryvalue">' + so.ftotal + '</span> (' + jarvis.string.formatNumber(ratio, 2) + '%)');
            else {
                $metric.find('.site').html('% of Total: <span class="summaryvalue">' + jarvis.string.formatNumber(ratio, 2) + '%</span> (' + series[1].ftotal + ')');
            }
        }
        else {
            ratio = 100;
            ...
            if (!jarvis.visualisation.dashboard.globalfilter || jarvis.visualisation.dashboard.globalfilter == '')
                $metric.find('.site').html('% of Total: <span class="summaryvalue">' + jarvis.string.formatNumber(ratio, 2) + '%</span> (' + so.ftotal + ')');
            else {
                $metric.find('.site').html('% of Total: <span class="summaryvalue">' + jarvis.string.formatNumber(ratio, 2) + '%</span> (' + series[1].ftotal + ')');
            }
        }*/
        ...
        //console.log('avg');
        //       $metric.find('.value').html(jarvis.string.formatNumber(so.avg, 2) + '%');
        //}
        // else {
 ...
 */
//new jarvis.visualisation.report.OverviewMetricBox().init();
