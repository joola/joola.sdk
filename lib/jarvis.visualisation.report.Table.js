...
jarvis.visualisation.report.Table = function (options, container) {
    ...
    this.colors = jarvis.colors; //['#058DC7', '#ED7E17', '#50B432', '#AF49C5', '#EDEF00', '#8080FF', '#A0A424', '#E3071C', '#6AF9C4', '#B2DEFF', '#64E572', '#CCCCCC' ];
...
jarvis.visualisation.report.Table.prototype.init = function (options, container) {
    ...
    $(jarvis.visualisation.report).bind('filter', function (e, options) {
        var parts = jarvis.visualisation.report.globalfilter.split('[AND]');
        var path = [];
        $(parts).each(function (index, part) {
            if (part != '') {
                ...
                if (part.indexOf('*=') > -1) {
                    ...
                    var _d = _.filter(jarvis.objects.Dimensions, function (d) {
                        return d.id == dID;
                    });
                    if (_d) {
                        _d = _d[0];
        ...
        if ($('.jarvis.breadcrumbs').length == 0) {
            var $breadcrumbsAppend = $('.secondary.btn-group').after('<div class="jarvis breadcrumbs offset3"></div>');
            //$('.tablecontrol').append('<div class="jarvis breadcrumbs offset3"></div>');
        }
        var $breadcrumbs = $('.jarvis.breadcrumbs');
        $breadcrumbs.empty();
        ...
        console.log($('.jarvis.breadcrumbs').length);
        
        $('.jarvis.breadcrumbs .crumb').each(function (index, item) {
            if (index < $('.jarvis.breadcrumbs .crumb').length - 1) {
                var $this = $(item);
                var $link = $($this.find('.value'));
                $link.off('click');
                $link.on('click', function (e) {
                    if ($this.attr('data-filter') == '') {
                        jarvis.visualisation.report.table.Filters = [];
                        jarvis.visualisation.report.timeline.Filters = [];
                    }
                    jarvis.visualisation.report.setFilter($this.attr('data-filter'));
                });
            }
        });
    });
...
//        _this.sortColumnIndex = 1;
        ...
        if (column.aggregation) {
            ...
            if (index == 0)
                ...
                $th = $('<th class="dimension" data-sortindex="' + sortIndex + '">' + dimension.name + '' +
                    '<i class="jarvis removesecondarydimension icon-remove" style=" margin-top: 2px; margin-left: 5px; cursor: pointer; "></i>' +
                    '' +
                    '</th>');
        ...
        $(_datatoshow).each(function (index, row) {
            ...
            if (_this.mode == 'pie') {
                ...
                var $_bar = $('<div class="barwrapper"><div class="percentagebar" style="width:' + width + '"></div>' +
                    '<td class="percentagecaption"><span >' + jarvis.string.formatNumber(row.Values[valIndex], 2) + '%</span></div>');
        ...
        $(_datatoshow).each(function (index, row) {
                ...
                $checkbox.on('click', function (e) {
                    ...
                    /*
                     ...
                     if ($checkbox.is(':checked')) {
                     jarvis.visualisation.report.addPartial(filter);
                     _this.Filters.push(filter);
                     }
                     else {
                     jarvis.visualisation.report.removePartial(filter);
                     if (_this.Filters.indexOf(filter) > -1) {
                ...
                if (_this.mode == 'perf') {
                    ...
                    try {
                        ...
                        $_bar = $('<div class="barwrapper"><div class="percentagebar" style="width:' + width + '"></div>' +
                            '<td class="percentagecaption"><span >' + jarvis.string.formatNumber(row_compare.Values[valIndex], 2) + '%</span></div>');
                    }
                    catch (ex) {
                        $td = $('<td class="special bar">0</td>');
                        $_bar = $('<div class="barwrapper"><div class="percentagebar" style="width:' + 0 + 'px"></div>' +
                            '<td class="percentagecaption"><span >' + jarvis.string.formatNumber(0, 2) + '%</span></div>');
                    }
                    $td.html($_bar);
    ...
    //$(jarvis).unbind('jarvis-reportTable-update');
    //$(jarvis).bind('jarvis-reportTable-update', function () {
    ...
    //$(jarvis).trigger('jarvis-reportTable-update');
...
jarvis.visualisation.report.Table.prototype.draw = function (Container) {
    ...
    $charttype.append('<button rel="tooltip" title="Table" class="btn btn_table active">' +
        '<img src="' + jarvis.contenthost + '/assets/img/glyphicons_114_list.png""/>' +
        '</i></button>');
    $charttype.append('<button rel="tooltip" title="Pie chart" class="btn btn_pie ' + (_this.DateBox.comparePeriod ? 'disabled' : '') + '" ' + (_this.DateBox.comparePeriod ? 'disabled="disabled"' : '') + '>' +
        '<img src="' + jarvis.contenthost + '/assets/img/glyphicons_042_pie_chart.png"/>' +
        '</button>');
    $charttype.append('<button rel="tooltip" title="Performance" class="btn btn_perf ' + (_this.DateBox.comparePeriod ? 'disabled' : '') + '" ' + (_this.DateBox.comparePeriod ? 'disabled="disabled"' : '') + '>' +
        '<img src="' + jarvis.contenthost + '/assets/img/glyphicons_110_align_left.png"/>' +
        '</button>');
    ...
    o = new jarvis.visualisation.picker.Dimensions({
        container: $item,
        prefix: '',
    ...
    var o = new jarvis.visualisation.container.Filter({
...
jarvis.visualisation.report.Table.prototype.setState = function (sender) {
    ...
    //jarvis.state[_this.uid()].metrics = _this.metrics;
    jarvis.state[_this.uid()].Filters = _this.Filters;
    //onsole.log(_this.mode);
    //jarvis.saveState('Table "' + _this.uid() + '" change');
};
...
jarvis.visualisation.report.Table.prototype.getState = function (sender) {
    ...
    if (jarvis.state[_this.uid()] != null) {
        ...
        //_this.metrics = jarvis.state[_this.uid()].metrics;
        //_this.Container = jarvis.state[_this.uid()].Container;
...
/*
 ...
 jarvis.visualisation.report.Table.init();
 */