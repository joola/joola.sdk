...
jarvis.visualisation.report.Table.prototype.draw = function (Container) {
    ...
    $charttype.append('<button rel="tooltip" title="Table" class="btn btn_table active">' +
        '<img src="' + jarvis.hostname + '/assets/img/glyphicons_114_list.png""/>' +
        '</i></button>');
    $charttype.append('<button rel="tooltip" title="Pie chart" class="btn btn_pie">' +
        '<img src="' + jarvis.hostname + '/assets/img/glyphicons_042_pie_chart.png"/>' +
        '</button>');
    $charttype.append('<button rel="tooltip" title="Performance" class="btn btn_perf">' +
        '<img src="' + jarvis.hostname + '/assets/img/glyphicons_110_align_left.png"/>' +
        '</button>');
    ...
    //fill metric lists
    /*
     $(jarvis.dataaccess.dimensions).each(function (index, dimension) {
     var item = $('<li class="jarvis dimensionlink" data-id="' + dimension.Id + '"><a >' + dimension.Name + '</a></li>');
     ...
     if (_this.dimensions.length > 1)
     $($tablecontrol.find('.secondarybutton')).html(_this.dimensions[_this.dimensions.length - 1].Name);
     else
     $($tablecontrol.find('.secondarybutton')).html('Secondary dimension...');
     $header.find('.secondarylist').append($dimlist);
     */
    var $item = $('<div class="jarvis picker dimensions" data-type="button"></div>');
    $($tablecontrol).find('.secondary').empty();
    $($tablecontrol).find('.secondary').append($item);
