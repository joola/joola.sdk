...
joolaio.provide('joolaio.visualisation.dashboard.Timeline');
 ...
 * @constructor
 */
joolaio.visualisation.dashboard.Timeline = function (options) {
    ...
    //joolaio.debug.log('INFO', 'joolaio.visualisation.dashboard.Timeline', 5, '...Constructor (' + executionTime + 'ms)');
};
 ...
 */
joolaio.visualisation.dashboard.Timeline.prototype.init = function (options, container) {
    var _this = this;
    var start = new Date().getMilliseconds();
    ...
    if (container)
        ...
        matchedContainers = $('.joolaio.dashboard.timeline');
    ...
    $(matchedContainers).each(function (index, item) {
        if (!$(this).parent().hasClass('prettyprint')) {
            joolaio.debug.log('INFO', 'joolaio.visualisation.dashboard.Timeline', 6, 'Applying to container (\'' + this.id + '\')');
...
joolaio.visualisation.dashboard.Timeline.prototype.fetch = function (sender, container) {
    if (!sender)
...
function fixdate(series) {
    ...
    $(series).each(function (i, series) {
        ...
        $(series.Rows).each(function (index, row) {
            ...
            value = joolaio.date.flatDate(value);
...
joolaio.visualisation.dashboard.Timeline.prototype.update = function (sender, metrics, series, container) {
    var _this = sender;
        ...
        chart: {
            ...
            events: {
                ...
                redraw: function () {
                    ...
                    $(joolaio).trigger('dashboard-timeline-redraw', [_series]);
            ...
            if (index > 0) {
                ...
                $(_series.Rows).each(function (index, item) {
                    ...
                    try {
                        ...
                        var y = parseFloat(item.Values[item.Values.length - 1]);
                        var actualdate = joolaio.date.flatDate(item.FormattedValues[0]);
                        ...
                        joolaio.debug.log('ERROR', 'joolaio.visualisation.dashboard.Timeline', 6, 'Failed to draw x point: ' + ex.message);
                    }
                });
...
joolaio.visualisation.dashboard.Timeline.prototype.baseHTML = function (sender) {
    var _this = sender;
...
joolaio.visualisation.dashboard.Timeline.prototype.draw = function (container) {
    var _this = this;
...
joolaio.debug.log('INFO', 'joolaio.Visualisation.Dashboard.Timeline', 6, 'JS source loaded');
