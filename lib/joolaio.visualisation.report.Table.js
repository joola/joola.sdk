...
//        _this.sortColumnIndex = 1;
  ...
  var checkedCount = $('.joolaio.report.jtable .checkfilter:checked').length;
  if (_this.options.defaultSelected > 0 && _this.options.defaultSelected > checkedCount && _this.Filters.length === 0) { //} && _this.initialLoad == 1) {
    var _limit = _this.options.defaultSelected;
    if (_limit > _data.length)
      _limit = _data.length;
    for (var i = 0; i < _limit; i++) {
  ...
  if (_this.options.defaultSelected > 0 && _this.Filters.length === 0) {
    for (var i = 0; i < _this.options.defaultSelected; i++) {
      var $checkbox = $($('.joolaio.report.jtable tr')[i]).find('.checkfilter');
      $checkbox.attr('checked', 'true');
      var _filter = $checkbox.attr('data-filter');
