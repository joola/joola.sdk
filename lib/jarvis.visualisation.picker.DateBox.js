...
jarvis.visualisation.picker.DateBox.init = function (options, container) {
    ...
    fork(calls, function () {
        ...
        //console.log(_this.base_fromdate);
        //console.log(_this.base_todate);
...
//console.log('test');
    ...
    $item = $('<table><tr valign=top>' +
        ...
        '<td class="datetable"><div class="datepicker"></div></td>' +
        '<td class="datetable"><div class="datepicker"></div></td>' +
        '<td class="datetable"><div class="datepicker"></div></td>' +
    ...
    $('.datetable-next .next').on('click', function (e) {
        ...
        //console.log('next month click', currentRightCellDate);

        //currentRightCellDate.setMonth(currentRightCellDate.getMonth() + 1);
        //console.log('next month click after update', currentRightCellDate);
        var selectedDate = new Date(currentRightCellDate);
        //console.log('next month click selected update', currentRightCellDate);
        $('.datepicker').each(function (index, item) {

            var localdate = new Date(selectedDate);
            //console.log(index,localdate);
            localdate.setMonth(localdate.getMonth() - (2 - index));
            $(item).datepicker('setDate', localdate);
        });
    });
...
//console.log('max',_this.base_todate);
    $('.datepicker').datepicker({
        dayNamesMin: ["S", "M", "T", "W", "T", "F", "S"],
        firstDay: 0,
        minDate: _this.min_date,
        maxDate: _this.max_date,
        ...
        onSelect: function (dateText, inst) {
            ...
            switch (_this.currentMode) {
                case 'base-from':
                    //console.log('changing to base-to');
                ...
                case 'base-to':
                    ...
                    if (_this.isCompareChecked) {
                        //console.log('changing to compare-from');
                        _this.currentMode = 'compare-from';

                    }
                    else {
                        //console.log('changing to base-from');
                ...
                case 'compare-from':
                    ...
                    //console.log('changing to compare-to');
    ...
    $('.datepicker').each(function (index, item) {
        //var selectedDate = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
    ...
    $('.jarvis.datebox .container').on('click', function (e) {
        ...
        if ($(this).hasClass('expanded')) {
            ...
            //$(".picker").css({left: ($(this).offset().left - $('.picker').width()) + 'px'});;
...
//    console.log(year + '-' + month + '-' + day + ' ' + hours + ':' + minutes + ' GMT');
    ...
    //fixedDate = new Date(fixedDate.getUTCDate());
    //console.log('date - ' + fixedDate);
    //console.log('utc date - ' + fixedDate_utc);
    ...
    //console.log('prev: ' + timestamp + ', fixed:' + fixedDate);

    //return new Date(jarvis.date.formatDate(timestamp, 'yyyy-mm-dd hh:nn:ss.000'));
...
jarvis.visualisation.picker.DateBox.formatDate = function (date) {
    ...
    //console.log(date + ' - ' + dDate + ' - ' + Highcharts.dateFormat('%b %e, %Y', dDate));
...
jarvis.visualisation.picker.DateBox.handleChange = function (options) {
    var _this = this;
    //console.log('Handle change: ' + _this.base_fromdate + '-' + _this.base_todate);
    //console.log('Handle change: ' + _this.compare_fromdate + '-' + _this.compare_todate);
    ...
    switch (this.currentMode) {
        case 'base-from':
            //console.log('drawing according to base-to');
            ...
            break;
        case 'base-to':
            //console.log('drawing according to base-from');
            ...
            break;
        case 'compare-from':
            //console.log('drawing according to compare-from');
            $($('.daterange.baserange .dateoption')[0]).removeClass('active');
            $($('.daterange.baserange .dateoption')[1]).removeClass('active');
            ...
            break;
        case 'compare-to':
            //console.log('drawing according to compare-to');
            $($('.daterange.baserange .dateoption')[0]).removeClass('active');
            $($('.daterange.baserange .dateoption')[1]).removeClass('active');
            $($('.daterange.comparerange .dateoption')[0]).removeClass('active');
...
jarvis.visualisation.picker.DateBox.updateLabels = function (sender, options) {
    ...
    //console.log(options.base_fromdate, options.base_todate);
...
jarvis.visualisation.picker.DateBox.getState = function (sender) {
    ...
    //console.log(new Date(jarvis.state.fromdate));
//console.log('test');
