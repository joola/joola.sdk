...
jarvis.visualisation.realtime.Panel = function (options) {
    ...
    try {
        ...
        try{
        this.panelID = jarvis.dataaccess.realtimepanels[0].ID; //this.getDefaultPanel();
        }
        catch(e){
            this.panelID = -1;
        }
    ...
    //jarvis.debug.log('INFO', 'jarvis.visualisation.realtime.Panel', 5, '...Constructor (' + executionTime + 'ms)');
};
...
jarvis.visualisation.realtime.Panel.prototype.init = function (options, container, drawWidgets, breakBindLoop, saveState) {
    ...
    try {
        ...
        if (this.panelID == -1)
        this.panelID = jarvis.objects.RealtimePanels[0].ID; //this.getDefaultPanel();
    }
...
jarvis.visualisation.realtime.Panel.prototype.setDisplay = function () {
    ...
    $('body').find('.jarvis.dashboard.panel').hide();
    $($('body').find('.jarvis.dashboard.panel')).find('.widgets').empty();
    $('body').find('.jarvis.picker.datebox').hide();
    $('.jarvis.realtime.panel').show();
    */
}
...
jarvis.visualisation.realtime.Panel.prototype.drawWidgets = function (sender, container, redraw) {
    ...
    //$(container).find('.jarvis.realtime.widgets').empty();
    $(container).empty();
    _html = '<div class="jarvis realtime row-top row fluid-row">&nbsp;' +
        '</div>';

    _html += '<div class="row fluid-row">' +
        '<div class="jarvis realtime column-left span4 columns ">&nbsp;' +
        '</div>' +
        '<div class="jarvis realtime column-right span8 columns">&nbsp;' +
        '</div>' +
        '</div>';
    ...
    $('.jarvis.realtime.column-left, .jarvis.realtime.column-right').sortable({
        connectWith:".columns",
        handle:'.header .move',
        placeholder:{
        ...
        switch (item.Type) {
            case 'Realtime Timeline':
                _html = '<div class="span3 columns maincounter">' +
                    '<div id="metricbox" class="jarvis realtime metricbox" data-metrics="' + item.PrimaryMetric.Name + '"></div>' +
                    '</div>' +
                    '<div class="span9 columns">' +
                    '<div id="timeline" class="jarvis realtime timeline" data-metrics="' + item.PrimaryMetric.Name + '"></div>' +
                    '</div>';
    ...
    if (redraw == true) {
        //console.log('true');
        //console.log(new jarvis.realtime.visualisation.MetricBox());
...
/*
 jarvis.visualisation.realtime.Panel.prototype.list = function (container) {
 ...
 jarvis.debug.log('INFO', 'Realtime.Visualisation.RealtimePanel', 6, 'Switching Panel');
 ...
 //jarvis.realtime.paused = false;

 sender.panelID = item.ID;
 jarvis.realtime.panelFilter = item.Segment;
 jarvis.realtime.processFilter();
 //sender.init(null, null, true, true);
 //sender.drawWidgets(sender, $('.jarvis.container.realtime'), true);
 });
...
jarvis.visualisation.realtime.Panel.prototype.get = function (sender, id) {
    if (id == -1)
        return;
    //var data = jarvis.dataaccess.fetch(this, '/engine/RealtimePanels.svc/get', {id:id}, null);
    //data = $.parseJSON(data.data);
...
jarvis.visualisation.realtime.Panel.prototype.getDefaultPanel = function (container) {
    //var data = jarvis.dataaccess.fetch(this, '/engine/RealtimePanels.svc/list', null, null);
    //data = $.parseJSON(data.data);
...
/*
var realtimePanel = new jarvis.visualisation.realtime.Panel();
$(document).ready(function (e) {
    if (jarvis.state.view == 'realtime'){

        var options = {panelID:jarvis.state.panelID};
